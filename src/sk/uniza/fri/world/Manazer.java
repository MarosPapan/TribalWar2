...
public class Manazer {
    ...
    private class ManazerMysi extends MouseAdapter {
        public void mouseClicked(MouseEvent event) {
            if (event.getButton() == MouseEvent.BUTTON1) {
                Manazer.this.posliSpravu("clickingOnLandScape", event.getX(), event.getY());
            }
        }
    }
    ...
    public Manazer() {
        ...
        Canvas.getCanvas().addKeyListener(new ManazerKlaves());
        Canvas.getCanvas().addTimerListener(new ManazerCasovaca());
        Canvas.getCanvas().addMouseListener(new ManazerMysi());
    }
